...
/**
 * @file
 * @brief contain windspeed report
 */
...
function plaatprotect_windspeed_page() {
		   ...
		   <script type="text/javascript" src="https://www.google.com/jsapi"></script>
			...
			function drawChart() {
				...
				while ($node = plaatprotect_db_fetch_object($result3)) {
					$page .= 'data.addColumn("number", "'.plaatprotect_db_zigbee($node->zid)->location.' [m/s]");'."\r\n";
				};
	...
	$page .= '<h1>'.t('SENSOR_TYPE_8').' '.plaatprotect_dayofweek($date).' '.$day.'-'.$month.'-'.$year.'</h1>';
...
function plaatprotect_windspeed() {
  ...
  global $pid;  
	...
	switch ($pid) {
		...
		case PAGE_WINDSPEED:
			return plaatprotect_windspeed_page();
			break;
	}
}
